...
const ProjectUpdate = ({ title }) => {
    ...
    useEffect(() => {
      const fetchData = async () => {
          try {
              ...
              if (project) {
                  console.log("getProject-----", project.developers[0].id);
                  setName(project.name)
                  setStartDate(project.startDate);
                  setDescriptions(project.description);
                  if (project.developers && project.developers.length > 0) {
                    const developerFullNames  = project.developers.map(dev => dev.id.fullName);
                    console.log("Developer IDs:", developerFullNames );
                    setDevelopers(developerFullNames );
                } else {
                    setDevelopers([]);
                }             
              }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            let project = { name, description, startDate, developers: developers.map(dev => ({ id: dev._id })) }
            console.log(project);
            let id = params.id
